...
currentMAC=$(ifconfig | grep ether | awk 'NR==2 {print $2}')
...
initialPrompt() {
    echo -e "Options: \n1: Only change your IP Address\n2: Only randomize your MAC Address\n3: Change both your IP and MAC\n4: Exit script"
    read -p "Please choose an option: " prompt
    case $prompt in
        "1") #only IP
            #promptChangeIP
            checkIP
...
##Not using this function in final version of script
promptChangeIP() {
    read -p "Would you like to change your IP address? (Y/N) " changeIPYesNo
...
##Not using this function in final version of script
promptChangeMAC() {
    read -p "Would you like to randomize your MAC address? (Y/N)" changeMACYesNo
case $changeMACYesNo in
    "Y" | "Yes" | "y" | "yes")
        changeMAC
        #echo "test"
        ;;
...
echo "Changing your MAC to a random address..."

interface=$(iwgetid | awk '{print $1}')
    
nmcli con delete $ssid
ifconfig $interface down
#ifconfig $interface hw ether $randomMAC
macchanger -r $interface
ifconfig $interface up


newCurrentMAC=$(ifconfig | grep ether | awk 'NR==2 {print $2}')
echo "Your new MAC address is: $newCurrentMAC"
}
